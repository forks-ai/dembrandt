...
function colorRole(hex, colorItem) {
  ...
  // Surface: very light or very dark AND high count (dominant background)
  if (lum > 0.85 || lum < 0.015) return 'surface';
  // Neutral: low saturation
  const max = Math.max(rgb.r, rgb.g, rgb.b) / 255;
  const min = Math.min(rgb.r, rgb.g, rgb.b) / 255;
  const saturation = max === 0 ? 0 : (max - min) / max;
  if (saturation < 0.15) return 'neutral';
  // Accent: chromatic, medium–high confidence
...
function hoverVariant(hex) {
  ...
  if (!rgb) return null;
  const lum = relativeLuminance(rgb);
  // Darken light colors, lighten dark ones by ~10%
  const factor = lum > 0.18 ? 0.88 : 1.15;
